...
def l1_regularization_gradient_descent(X: np.ndarray, y: np.ndarray, alpha: float = 0.1, learning_rate: float = 0.01, max_iter: int = 1000, tol: float = 1e-4) -> tuple:
    ...
    for i in range(max_iter):
        yhat = X @ weights + bias 
        loss = 1 / (2 * n_samples) * np.sum((yhat - y)**2) + alpha * np.sum(np.abs(weights))

        dL_dw = (1/n_samples) * (X.T @ (yhat - y)) 
        dL_db = (1/n_samples) * np.sum(yhat - y)

        w_temp  = weights - learning_rate * dL_dw
        w_new = soft_threshold(w_temp, learning_rate * alpha)
        b_new = bias - learning_rate * dL_db

        if np.all(np.abs(w_new - weights) < tol):
            weights = w_new
            bias = b_new
            break
